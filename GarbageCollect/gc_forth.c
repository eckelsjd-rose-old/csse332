...
void handle_alloc_begin() {
	struct mem_rec *record = malloc(sizeof(struct mem_rec));
	record->start = forth.here;
	record->accessible = false;
	array_list_add(regions,(void*) record);
	// printf("Alloc address: %p\n",record->start);
}

void free_mem_rec(void* data) {
	free(data);
}

void handle_alloc_end() {
	struct mem_rec* record = array_list_get_idx(regions, array_list_length(regions)-1);
	record->end = forth.here;
...
int compute_alloced_size() {
	...
	int size = 0;
	for (int i=0; i < array_list_length(regions); i++) {
		struct mem_rec* record = array_list_get_idx(regions, i);
...
int compute_num_regions() {
    return array_list_length(regions);
}

// determines if a void* ptr is pointing to a valid memory region
struct mem_rec* find_region(void* ptr) {
	for (int i=0; i< array_list_length(regions); i++) {
		struct mem_rec *record = array_list_get_idx(regions,i);
		if (ptr >= record->start && ptr < record->end) {
			//printf("Record added: %d. Start: %p\n",i,record->start);
			return record;
		}
	}
	return NULL; // didn't find a record
}

void free_queue(void* data) {
	// do nothing
}

int compute_unrefed_size() {

	// create a 'queue' array list of mem_rec* regions to search
	array_list_free_fn *func = &free_queue;
	array_list *queue = array_list_new(func);

	// create mem record for stack
	struct mem_rec *stack_rec = malloc(sizeof(struct mem_rec));
	stack_rec->start = forth.stack_top;
	stack_rec->end = forth.stack_bot;
	array_list_add(queue,stack_rec);

	// find mem record for forth.latest and add to queue
	struct mem_rec *latest_rec = find_region((void*) forth.latest);
	// printf("forth latest: %p\n",forth.latest);
	array_list_add(queue,latest_rec);

	// mark all as inaccessible and not visited
	for (int i=0; i<array_list_length(regions); i++) {
		struct mem_rec *record = array_list_get_idx(regions,i);
		record->accessible = false;
		// printf("Record %d. Start: %p. End: %p\n",i,record->start,record->end);
	}
	
	latest_rec->accessible = true;
	
	while (array_list_length(queue) > 0) {
		struct mem_rec *record = array_list_get_idx(queue,0);
		// printf("start: %p.\n",record->start);
		if (record != NULL) {
			for (void *ptr=record->start; ptr < record->end; ptr++) {
				void* potential_ptr = *((void**) ptr);
				struct mem_rec *data = find_region(potential_ptr);
				if (data != NULL) {
					if (!data->accessible) {
						array_list_add(queue,data);
						data->accessible = true;
						//printf("Record added: %p. Accessible: %d.\n",potential_ptr,data->accessible);
					}
				}
			}
		}
	...
	array_list_free(queue);

	// count up bytes for unreferenced regions
	// printf("COUNT BYTES:\n");
	int bytes = 0;
	for (int i=0; i < array_list_length(regions); i++) {
		struct mem_rec *record = array_list_get_idx(regions,i);
		// printf("Record: %d. Access: %d\n",i,record->accessible);
		if (!record->accessible) {
			bytes = bytes + (record->end - record->start);
		}
	}
    return bytes;
}
...
void initialize_forth_for_test() {
	...
	// initialize regions array_list
	array_list_free_fn *func = &free_mem_rec;
	regions = array_list_new(func);
