...
os_bool_t fetch_inode(os_uint32_t inode_number, int fd,
  ...
  // the 10th?  the 100th?)
  os_uint32_t offset_within_blockgroup;  // you must fix this.
  if (inode_number % metadata->inodes_per_group == 0) {
    offset_within_blockgroup = metadata->inodes_per_group - 1;
  } else {
    offset_within_blockgroup = inode_number % metadata->inodes_per_group - 1;
  }
  ...
  size_t inode_byte_offset = (offset_within_blockgroup % num_inodes_per_block) * sizeof(struct os_inode_t);
  
  // Step 6. Use lseek() Sand read() to read the inode off of disk
  // and into "inode", given the inodes block number and
  // byte offset within that block that you calculated earlier.
  int absolute_byte_offset = inode_block_num * metadata->block_size + inode_byte_offset;

  lseek(fd, absolute_byte_offset, SEEK_SET);
...
void calculate_offsets(os_uint32_t blocknum,
  ...
  if (blocks_left < (blocksize/4)*(blocksize/4)) {
    ...
    *direct_num = *triple_index = -1;
	if (blocknum == 1643) {
		printf("For blocknum: %d, my offsets are direct=%d, si=%d, di=%d, ti=%d\n",blocknum,*direct_num,*indirect_index,*double_index,*triple_index);
	}
    return;
  }
  ...
  if (blocks_left < (blocksize/4)*(blocksize/4)*(blocksize/4)) {
    ...
    *triple_index = blocks_left / ((blocksize/4)*(blocksize/4));
    int new_blocks_left = blocks_left - (*triple_index * ((blocksize/4)*(blocksize/4)));
    *double_index = new_blocks_left / (blocksize/4);
    *indirect_index = new_blocks_left % (blocksize/4);
    return;
  }
...
os_bool_t file_read(int fd, int file_inode_num,
  ...
  if (!((inode->i_mode & EXT2_S_IFREG) || (inode->i_mode & EXT2_S_IFDIR) || (inode->i_mode & EXT2_S_IFLNK))) {
    return FALSE;
  } 
  // Step 3.  If the file size is 0, return TRUE.
  if (inode->i_size == 0) { return TRUE; }
  ...
  *buffer = malloc(malloc_size);
  ...
  //printf("File size: %d. Malloced: %d\n",inode->i_size,malloc_size);
  ...
  for (int i = 0; i < malloc_size/metadata->block_size; i++) {
    file_blockread(*inode, fd, metadata, i, ptr);
    ptr += metadata->block_size;
  }
  // unsigned char* output = *buffer;
  // os_uint32_t blocks_read = 0;
  // while(blocks_read*metadata->block_size < inode->i_size) {
  //   int result = file_blockread(*inode,
  //                               fd,
  //                               metadata,
  //                               blocks_read,
  //                               output + blocks_read * metadata->block_size);
  //   if(result < 0) {
  //     free(output);
  //     return FALSE;
  //   }
...
os_uint32_t scan_dir(unsigned char *directory,
  ...
  while (current_offset < directory_length) {
    ...
    struct os_direntry_t *current_entry = (struct os_direntry_t *) (directory + current_offset);
    ...
    for (int i = 0; i < strlen(filename); i++) {
      if (current_entry->file_name[i] != filename[i]) {
        found = 0;
        break;
      }
    }
    if (found) { return current_entry->inode; }
...
int path_helper(char* path, int fd, struct os_fs_metadata_t *metadata, int inode_num) {
  ...
  strncpy(path_copy, path, strlen(path)+1); // path copy gives a pointer to the next directory to read
  char* path_remaining = path_copy; // path_remaining will trim off the directory and go past the next slash (if possible)
  char* next_slash = strchr(path_copy, '/');
  if (next_slash == NULL) {found_file = 1; } // we have found the file if there are no more slashes
  else {
    next_slash[0] = '\0';
    path_remaining = next_slash + 1;
  }
  ...
  //1. Use file_read to read the current directory data into inode and buffer
  file_read(fd, inode_num, metadata, &inode, &buffer);
  //2. Use scan_dir to search for the entry you're looking for
  int next_inode = scan_dir(buffer,inode.i_size, path_copy);
  //3. That will become the directory you'll use in the next iteration
  if (found_file) {return next_inode; } // base case for recursion

  return path_helper(path_remaining, fd, metadata, next_inode);
}
...
os_uint32_t path_to_inode_num(char* path, int fd, struct os_fs_metadata_t *metadata) {
  ...
  // recursively trim path until we have found the file
  return path_helper(path + 1, fd, metadata, root);
}
