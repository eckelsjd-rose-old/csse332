...
int main() {
    ...
    char cmd[512];
        ...
        if (cmd[0] == 't' && cmd[1] == 'y' && cmd[2] == 'p' && cmd[3] == 'e'){
			...
			filename[j] = '\0';
			interrupt(0x21, 3, filename, buffer, 0); /* readFile filename into buffer */
		...
		/* execute progra */
			...
			filename[j] = '\0';
			interrupt(0x21, 4, filename, 0, 0); /* executeProgram */
		} 
		/* dir */
		...
		/* kill */
			...
			int pid = cmd[5] - '0';
			interrupt(0x21,9,pid,0,0);	
		} else if (cmd[0] == 'e' && cmd[1] == 'x' && cmd[2] == 'e' && cmd[3] == 'c' && cmd[4] == 'f' && cmd[5] == 'o' && cmd[6] == 'r' && cmd[7] == 'e' && cmd[8] == 'g' && cmd[9] == 'r' && cmd[10] == 'o' && cmd[11] == 'u' && cmd[12] == 'n' && cmd[13] == 'd') {
			j = 0;
			while (cmd[15+j] != '\0' && j < 6 && cmd[15+j] != '\n') {
				filename[j] = cmd[15+j];
				j++;
			}
			filename[j] = '\0';
			interrupt(0x21, 10, filename, 0, 0); /* wait */
		}
		/* bad command */
		else {
