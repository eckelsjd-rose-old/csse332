...
void pri_sem_init(struct pri_sem_t* pri_sem, int value) {
    // Implement me
	memset(pri_sem->priority,0,sizeof(pri_sem->priority));
	pri_sem->value = value;
	for (int i=0;i<MAX_PRI_SEM_SIZE;i++) {
		sem_init(&pri_sem->sems[i],0,0);
	}
	sem_init(&pri_sem->mutex,0,1);
...
void pri_sem_wait(struct pri_sem_t* pri_sem, int priority) {
    // Implement me
	sem_wait(&pri_sem->mutex); // lock
	if (pri_sem->value == 0) {
		for (int i=0;i<MAX_PRI_SEM_SIZE;i++) {
			if (pri_sem->priority[i] == 0) { // find open slot
				pri_sem->priority[i] = priority; // claim slot
				sem_post(&pri_sem->mutex); // free the lock on pri_sem before waiting
				sem_wait(&pri_sem->sems[i]); // sem_post will have obtained the lock when this line returns
				pri_sem->priority[i] = 0; // reopen slot
				break;
			}
		}
	} else {
	...
	sem_post(&pri_sem->mutex); // reopen lock
}
...
void pri_sem_post(struct pri_sem_t* pri_sem) {
	...
	// find max priority
	for (int i=0;i<MAX_PRI_SEM_SIZE;i++) {
		if (pri_sem->priority[i] > max_pri) {
			max_pri = pri_sem->priority[i];
			max_idx = i;
		}
	}
	if (max_idx != -1) {
		pri_sem->value++;
		sem_post(&pri_sem->sems[max_idx]);
	}
	// don't unlock mutex; sem_wait will handle this	
}
...
Meanwhile, THREAD B (A?) knows it must have "aquired" the mutex when it
